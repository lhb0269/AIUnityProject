...
**동기 및 필요성**
- 초기 프로젝트는 Singleton과 Static method에 의존하여 테스트 간 종속성 발생
- VContainer DI 패턴 도입으로 테스트 격리성과 유지보수성 향상
...
| 지표 | 값 |
|-----|-----|
| **총 버튼 수** | 34개 (메인 메뉴) + 15개 (햄버거 메뉴) = 49개 |
...
| **시각적 UI 검증** | 49개 버튼 | 0개 | 0% | ⚠️ 수동 검수 |
| **성능 테스트** | - | - | 0% | ⚠️ 범위 외 |

### 종합 커버리지

```
UI 계층 자동화 커버리지: 100% (106/106 체크포인트)
  - 버튼 클릭: 49/49 ✅
  - 팝업 열기 (Mock): 23/23 ✅
  - DI 주입 검증: 55/55 ✅
  - 로그 검증: 25/25 ✅
...
|-------------|------------|----------|---------|---------|
| **MainMenuControllerTests** | 34개 | 34개 버튼 (20개 팝업 + 13개 일반 + 1개 통합) | 517줄 | ~4초 |
| **HamburgerMenuPopupTests** | 21개 | 15개 버튼 (12개 일반 + 3개 팝업 + 2개 Lifecycle + 1개 DI + 2개 Edge Case + 1개 통합) | 555줄 | ~2초 |
| **합계** | **55개** | **49개 버튼** | **1,072줄** | **~6초** |
...
| 49개 버튼 클릭 및 팝업 검증 | 75초 (각 ~1.5초) | 6초 | 69초 | 92% |
