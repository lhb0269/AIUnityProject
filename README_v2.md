...
**1. 일반 버튼 (25개)**
- 버튼 클릭 시 해당 이벤트 핸들러가 호출되는지 검증합니다.
- 로그 메시지가 올바르게 출력되는지 검증합니다.
- 예: 컨트롤 버튼, 챕터 선택, 스킬 버튼 등

**2. 팝업 버튼 (23개)**
- 버튼 클릭 시 해당 팝업이 화면에 생성되는지 검증합니다.
- 팝업의 닫기 버튼이 정상 동작하는지 검증합니다.
- UIManager의 팝업 카운트가 올바른지 검증합니다.
- 예: 설정 팝업, 상점 팝업, 캐릭터 정보 등

**3. 팝업 스택 관리**
- LIFO(Last In First Out) 구조로 중첩 팝업을 관리합니다.
- 햄버거 메뉴 → 마을 팝업 → 닫기 시 햄버거 메뉴는 유지 되어야 합니다.
- 팝업 중복 열기 방지 로직을 검증합니다.
...
|---------|---------|----------|----------|
| **1. 단일 팝업 열기/닫기** | High | 20개 팝업 | ① 팝업 생성 확인<br>② 팝업 카운트 1 확인<br>③ 닫기 버튼 클릭 시 팝업 제거<br>④ 팝업 카운트 0 확인 |
| **2. 중첩 팝업 스택** | High | 3개 팝업 | ① LIFO 구조 확인<br>② 부모 팝업 유지 확인<br>③ 자식 팝업만 닫기<br>④ 중복 열기 방지 |
| **3. 일반 버튼 동작** | High | 25개 버튼 | ① 버튼 클릭 이벤트 호출<br>② 로그 메시지 출력<br>③ 게임 상태 변경 없음 확인 |

### 우선순위 정의 및 분포

| 우선순위 | 테스트 케이스 수 | 비율 | 설명 | 예시 |
|---------|---------------|------|------|------|
| **Critical** | 0개 | 0% | 팝업 열기/닫기 실패 시 게임 크래시 또는 플레이 불가 | 설정 팝업 열기 시도 -> 크래시 |
| **High** | 62개 | 100% | 로그 미출력,팝업 미 노출 | 스킬1 버튼 클릭 -> 스킬 시전 로그 미노출(실제 스킬은 발동), 설정 버튼 클릭 -> 게임 설정 팝업 미노출 |
| **Medium** | 14개 | 22.6% | 애니메이션 문제(Asset 문제)|
...
| 개선 항목 | 이전 | 이후 | 효과 |
|---------|-----|-----|------|
| **싱글톤 관리** | 테스트 간 간섭 | ResetForTesting() 메서드 추가 |
